...
const char* ParseFlagValue(const char* str, const char* flag, bool def_optional) {
    ...
    // Skip the flag name
    const auto* const flag_end = str + flag_len;
...
bool ParseUInt16(const std::string& src, const char* str, uint16* value) {
    ...
    if (*end != '\0') {
        std::cerr << src
                  << " is expected to be a 16-bit unsigned integer, "
    ...
    auto result = static_cast<uint16>(ulong_val);
    if (std::numeric_limits<uint64>::min() == ulong_val ||
        std::numeric_limits<uint64>::max() == ulong_val || result != ulong_val) {
        std::cerr << src
                  << " is expected to be a 16-bit unsigned integer,"
                     " but actually has the value "
                  << str << " which overflows\n";
...
bool ParseUInt16Flag(const char* str, const char* flag, uint16* value) {
    ...
    // Set |value| to the value of the flag
    return ParseUInt16(std::string("The value of flag --") + flag, value_str, value);
}
