...
#include "file/file_enumerator.h"
...
class FileEnumeratorTest : public testing::Test {
  protected:
    virtual void SetUp() {
        tmp_dir = GetTempDir().append("fe-test");
        ASSERT_TRUE(CreateDirectory(tmp_dir));
    }
...
TEST_F(FileEnumeratorTest, EmptyDirectory) {
    FileEnumerator fe(tmp_dir, true, FileType::FILES | FileType::DIRECTORIES);
    EXPECT_EQ("", fe.next().value());
    EXPECT_EQ("", fe.next().value());
}

TEST_F(FileEnumeratorTest, EmptyDirectoryWithDotDot) {
    FileEnumerator fe(tmp_dir, false,
                      FileType::FILES | FileType::DIRECTORIES | FileType::INCLUDE_DOT_DOT);
    EXPECT_EQ(tmp_dir.append("..").value(), fe.next().value());
    EXPECT_EQ("", fe.next().value());
}

TEST_F(FileEnumeratorTest, EnumerateDirectories) {
    auto dir1 = tmp_dir.append("dir1");
    auto dir2 = tmp_dir.append("dir2");
    auto dir2_inner = tmp_dir.append("inner");
    EXPECT_TRUE(CreateDirectory(dir1));
    EXPECT_TRUE(CreateDirectory(dir2));
    EXPECT_TRUE(CreateDirectory(dir2_inner));

    FileEnumerator fe(tmp_dir, true, FileType::DIRECTORIES);
...
TEST_F(FileEnumeratorTest, EnumerateDirectoriesNonRecursive) {
    ...
    FileEnumerator fe(tmp_dir, false, FileType::DIRECTORIES);
    FindResultCollector cl(&fe);
    EXPECT_TRUE(cl.hasFile(dir1));
    EXPECT_TRUE(cl.hasFile(dir2));
    EXPECT_EQ(2, cl.size());
...
TEST_F(FileEnumeratorTest, EnumerateDirectoriesNonRecursiveWithDotDot) {
    ...
    FileEnumerator fe(tmp_dir, false, FileType::DIRECTORIES | FileType::INCLUDE_DOT_DOT);
    FindResultCollector cl(&fe);
    EXPECT_TRUE(cl.hasFile(dir1));
    EXPECT_TRUE(cl.hasFile(dir2));
    EXPECT_TRUE(cl.hasFile(tmp_dir.append("..")));
    EXPECT_EQ(3, cl.size());
...
TEST_F(FileEnumeratorTest, EnumerateDirectoriesWithPattern) {
    auto dir1 = tmp_dir.append("dir1");
    auto dir2 = tmp_dir.append("dir2");
    auto dir3 = tmp_dir.append("foo1");
    EXPECT_TRUE(CreateDirectory(dir1));
    EXPECT_TRUE(CreateDirectory(dir2));
    EXPECT_TRUE(CreateDirectory(dir3));

    FileEnumerator fe(tmp_dir, true, FileType::FILES | FileType::DIRECTORIES, "dir*");
    FindResultCollector cl(&fe);
    EXPECT_TRUE(cl.hasFile(dir1));
    EXPECT_TRUE(cl.hasFile(dir2));
...
TEST_F(FileEnumeratorTest, EnumerateFiles) {
    auto dir1 = tmp_dir.append("dir1");
    EXPECT_TRUE(CreateDirectory(dir1));
    auto dir1file = tmp_dir.append("dir1").append("file1.csv");
    createFile(dir1file);

    auto dir2 = tmp_dir.append("dir2");
    EXPECT_TRUE(CreateDirectory(dir2));
    auto dir2file = tmp_dir.append("dir2").append("dir2file.txt");
    createFile(dir2file);

    auto dir2_inner = dir2.append("inner");
    EXPECT_TRUE(CreateDirectory(dir2_inner));
    auto dir2innerFile = dir2_inner.append("innerfile.ini");
    createFile(dir2innerFile);

    FileEnumerator fe(tmp_dir, true, FileType::FILES);
    FindResultCollector cl(&fe);
    EXPECT_TRUE(cl.hasFile(dir1file));
    EXPECT_TRUE(cl.hasFile(dir2file));
    EXPECT_TRUE(cl.hasFile(dir2innerFile));
    EXPECT_EQ(3, cl.size());

    // clean up after test is finished
    EXPECT_TRUE(File::remove(dir1file));
    EXPECT_TRUE(File::remove(dir1));
    EXPECT_TRUE(File::remove(dir2innerFile));
